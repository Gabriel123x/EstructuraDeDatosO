...
namespace E3_1.AcevedoEnsisoPedroGabriel
{
    class Program
    {
        ...
        {
            ArrayList Tecnologico = new ArrayList();//Lista de Objetos tipo Salon esto ayudo bastante a hacer el programa mas sencillo
            ...
            do//aqui comienza el ciclo del programa siempre y cuando Activo sea verdadera el programa seguira ejecutandose
            {
                try
                {
                    ...
                    {                                    //dependiendo de cuantas clases introdujo el usuario son las veces que el ciclo generara salones
                        Console.WriteLine("Cual es la clase que se imparte en el salon {0}? clases restantes {1}",(i + 1),TotalClases - (i + 1));
                        nombre = Console.ReadLine();//primero el usuario introduce el nombre de la clase
                        Aula = new Salon(nombre);//aqui creamos el salon con los datos introducidos
                        Tecnologico.Add(Aula);//agregamos el salon a la lista
                    }
                    Console.Clear();
                    foreach(Salon i in Tecnologico)//en este ciclo usamos los metodos PedirAlumnos, PedirCalificaciones y LeerNumeroDeAlumnos de cada objeto Salon en la lista
                    {
                        i.LeerNumeroDeAlumnos();
                        i.PedirAlumnos();
